...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        self.inference_engine = LLM(
            ...
            seed=int(os.getenv("RANK", "0")) // tensor_parallel_size, ## make sure sampling results are the same across workers (to pass the vllm config assertion)
        )
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            try:
                ...
                print(f"Error: {e}")
                if hasattr(vllm_inputs, 'shape'):
                    print(f"vllm_inputs shape: {vllm_inputs.shape}")
                elif hasattr(vllm_inputs, 'size'):
                    print(f"vllm_inputs size: {vllm_inputs.size()}")
